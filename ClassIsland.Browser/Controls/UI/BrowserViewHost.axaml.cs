...
namespace ClassIsland.Browser.Controls.UI;

/// <summary>
/// 浏览器端视图宿主。从 <c>ClassIsland.Android.Controls.UI.AndroidViewHost</c> 移植，
/// 去掉 Activity 相关的生命周期耦合——浏览器端只有唯一一个常驻宿主。
/// </summary>
public partial class BrowserViewHost : UserControl, IViewHost
{
    ...
    /// <summary>
    /// 在应用启动完成后填入课表条。必须等到此时——<see cref="ScheduleBarHost"/> 在构造期即解析
    /// DI 服务，而本宿主早于 <c>App.Init()</c> 创建。
    /// </summary>
    public void AttachScheduleBar()
    {
        if (ScheduleBarContainer.Content != null)
        {
            return;
        }

        ScheduleBarContainer.Content = new ScheduleBarHost();
    }
    ...
    private async Task ShowViewCore(ViewBase view, ViewBase? owner, bool modal)
    {
        ...
        // 首个视图之后一律压栈，否则后续视图会互相替换：导航栈里始终只有一页，
        // HideView 便走不到 PopAsync，Popped 事件不触发，视图的激活/停用无法配对，
        // 最终 ViewBase.Hide() 因 AssociatedViewHost 为空而抛异常。
        // 桌面端 splash 一定出现，所以原实现只在 SplashView 时置位也能工作；
        // 浏览器端 IsSplashEnabled 默认为 false，SplashView 永不出现。
