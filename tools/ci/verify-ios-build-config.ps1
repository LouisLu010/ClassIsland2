...
Assert-True ($automationIntentSourceReferences -eq 2) "The automation intent must be compiled exactly once into the main-app Swift bridge."
$liveActivityExtensionSources = [regex]::Match($nativeProjectText, '(?s)A30000000000000000000004 /\* Sources \*/ = \{.*?\n\t\t\};')
Assert-True ($liveActivityExtensionSources.Success -and -not $liveActivityExtensionSources.Value.Contains('ClassIslandAutomationIntent.swift')) "The Live Activity extension must not register a second automation intent with isolated UserDefaults."
